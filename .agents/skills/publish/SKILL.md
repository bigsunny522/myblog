...
`content/drafts/<slug>.mdx` を `content/posts/` へ公開する。**マージは行わない** — PR作成までがこのスキルの範囲(main へのマージは明示的な指示があった時のみ、という AGENTS.md の Git ワークフローに従う)。
...
`public/images/posts/<slug>/` に `coverImage` と本文中で参照している画像がすべて存在するか確認する(`npm run check:posts` でも後述の手順5で機械チェックされるが、先に目視確認しておくと手戻りが少ない)。元画像が7〜10MBを超える場合は `npm run optimize:images -- posts/<slug>` で圧縮する(AGENTS.md の画像運用ルール参照)。
